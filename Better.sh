...
do_video(){
    ...
    youtube-dl --id --write-auto-sub --skip-download ""${entry}"" | grep "info|WARNING"
    if grep "<c> slime" "${entry}.en.vtt"; then
        youtube-dl --id -f 18 ""${entry}""
        grep "<c> slime" "${entry}.en.vtt" | sed -r "s/.*(<[0-9][0-9]:[0-9][0-9]:[0-9][0-9]\.[0-9][0-9][0-9]><c> slime<\/c><[0-9][0-9]:[0-9][0-9]:[0-9][0-9].[0-9][0-9][0-9]>).*/\1/g" | grep -v '[abdfghjknopqrtuvwxyz]' | sed -e "s/><c> slime<\/c></,/g" -e "s/[<>]//g" > ${entry}.times
        cat ${entry}.times
        local O=0
        while read time; do 
            (( O += 1 )) 
